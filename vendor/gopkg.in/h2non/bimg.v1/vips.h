...
int
vips_init_image (void *buf, size_t len, int imageType, double pdfDPI, VipsImage **out) {
	int code = 1;
	...
	if (imageType == JPEG) {
		...
		code = vips_pdfload_buffer(buf, len, out, "access", VIPS_ACCESS_RANDOM, "dpi", pdfDPI, NULL);
